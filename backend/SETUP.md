...
This script prompts for `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `SUPABASE_ANON_KEY`, and an optional `DATABASE_URL`,
stores them in a gitignored `.env`, initializes the schema (if `DATABASE_URL` is provided), and runs backend tests.
